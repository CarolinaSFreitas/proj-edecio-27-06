...
export async function filmeCreate(req, res) {
  const { titulo, genero, duracao, preco, foto, sinopse, admin_id } = req.body

  if (!titulo || !genero || !duracao || !preco || !foto || !sinopse || !admin_id) {
  ...
  try {
    const filme = await Filme.create({
      titulo, genero, duracao, preco, foto, sinopse, admin_id
    })
...
export async function filmeDestaqueUpdate(req, res) {
  ...
  try {
    ...
    const destacaFilme = await Filme.update({ destaque: !filme.destaque },
      {
        where: { id }
      })
    res.status(200).json(destacaFilme)
  } catch (error) {
    res.status(400).send(error)
  }
}
